...
// Set default panel
if ($panel == '' && in_array($widget->position, array('top-a', 'top-b', 'top-c', 'top-d', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'bottom-offset', 'footer', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('headerbar', 'footer-left', 'footer-right', 'offcanvas'))) {
    $panel = 'uk-panel';
}
// Set badge
...
// Set title
if (in_array($widget->position, array('headerbar', 'more', 'search', 'footer-left', 'footer-right', 'modal'))) {
    $title = '';
...
if ($widget->menu) {
    ...
    if (isset($params['menu'])) {
        ...
        if ($this['config']->get('navigation_style') !== 'default') {
            $widget->nav_settings["dropdown"] = array("pos" => 'bottom-center');
        }
    } else if (in_array($widget->position, array('footer-left', 'footer-right'))) {
        $renderer = 'subnav';
        $widget->nav_settings["modifier"] = "uk-subnav-line";
		if (in_array($widget->position, array('footer-left', 'footer-right'))) $widget->nav_settings["modifier"] .= " uk-flex-center";
...
// Render widget
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'search', 'fixed-bar', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
    echo $content;
